...
For interview/demo readiness, there’s an end-to-end retrieval evaluation pipeline:

- Generate synthetic eval queries per chunk: [`steps.evaluation.generate_dataset.generate_eval_dataset`](steps/evaluation/generate_dataset.py)
- Compute metrics (mean recall/precision @ k): [`steps.evaluation.evaluate_retrieval.retrieval_evaluation`](steps/evaluation/evaluate_retrieval.py)
- Persist runs to Mongo: [`publish_assist.domain.eval.EvalRun`](publish_assist/domain/eval.py)
...
Common endpoints:
- `GET /v1/health` → [api/routes/health.py](api/routes/health.py)
- `POST /v1/ingest` → [api/routes/ingest.py](api/routes/ingest.py) (starts ingestion job)
- `POST /v1/generate` → [api/routes/generate.py](api/routes/generate.py) (starts generation job)
- `GET /v1/jobs/{job_id}` → [api/routes/jobs.py](api/routes/jobs.py) (poll job + pipeline status)

Job schema (progress, errors, timing): [`api.schema.job.JobDoc`](api/schema/job.py)

---

## UI (Streamlit) (To be upgraded to ts, react)
...
Settings are defined in [`publish_assist.settings.Settings`](publish_assist/settings.py) and loaded from `.env` (see [.env](.env)).
...
- `QDRANT_URL`, `QDRANT_API_KEY` (optional), `USE_QDRANT_CLOUD` (optional)
- `TEXT_EMBEDDING_MODEL_ID` (default: `sentence-transformers/all-MiniLM-L6-v2`)
